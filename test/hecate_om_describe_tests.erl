-module(hecate_om_describe_tests).

-include_lib("eunit/include/eunit.hrl").

%% `hecate_om:service_module/0' reads this exact `persistent_term' key
%% -- set directly here rather than via `hecate_om:boot/2' (which also
%% starts stores/pubsub/HTTP listeners this suite has no business
%% touching just to exercise a stateless read). Mirrored from
%% `hecate_om.erl''s own `?SERVICE_MODULE_KEY' -- if that atom ever
...
capability_for_is_undefined_when_neither_callback_is_exported() ->
    ?assertEqual(undefined,
                 hecate_om_describe:capability_for(hecate_om_describe_fake_none, <<"svc">>)).

capability_for_is_present_when_only_rpc_is_exported() ->
    ?assertMatch(#{name := <<"svc.describe_capabilities">>},
                 hecate_om_describe:capability_for(hecate_om_describe_fake_rpc_only, <<"svc">>)).

capability_for_is_present_when_both_are_exported() ->
    ?assertMatch(#{name := <<"other-svc.describe_capabilities">>},
                 hecate_om_describe:capability_for(hecate_om_describe_fake_full, <<"other-svc">>)).

capability_for_has_no_auth_key_by_default() ->
    Cap = hecate_om_describe:capability_for(hecate_om_describe_fake_full, <<"svc">>),
    ?assertNot(maps:is_key(auth, Cap)).

set_none() ->
    persistent_term:put(?SERVICE_MODULE_KEY, hecate_om_describe_fake_none).

clear_service_module(_) ->
    persistent_term:erase(?SERVICE_MODULE_KEY).

describe_reads_the_current_service_module() ->
    persistent_term:put(?SERVICE_MODULE_KEY, hecate_om_describe_fake_full),
    ?assertEqual({ok, #{
        rpc    => [#{name => <<"svc.chat">>, description => <<"Chat completion">>}],
        pubsub => [#{topic => <<"svc.events">>, description => <<"Lifecycle events">>}]
    }}, hecate_om_describe:describe(ignored)).

describe_defaults_missing_callbacks_to_empty_lists() ->
    persistent_term:put(?SERVICE_MODULE_KEY, hecate_om_describe_fake_rpc_only),
    ?assertEqual({ok, #{
...
describe_ignores_its_payload_argument() ->
    ...
    ?assertEqual({ok, #{rpc => [], pubsub => []}},
                 hecate_om_describe:describe(#{some => <<"payload">>})).
