...
-module(hecate_om_health_listener_tests).

-include_lib("eunit/include/eunit.hrl").

unset_health_ip_gives_the_port_only_test() ->
    ?assertEqual([{port, 8470}], hecate_om_sup:health_socket_opts(8470, undefined)).

empty_health_ip_counts_as_unset_test() ->
    ?assertEqual([{port, 8470}], hecate_om_sup:health_socket_opts(8470, "")),
    ?assertEqual([{port, 8470}], hecate_om_sup:health_socket_opts(8470, <<>>)).

health_ip_as_a_string_is_parsed_test() ->
    ?assertEqual([{port, 8470}, {ip, {127, 0, 0, 1}}],
                 hecate_om_sup:health_socket_opts(8470, "127.0.0.1")),
    ?assertEqual([{port, 8470}, {ip, {127, 0, 0, 1}}],
                 hecate_om_sup:health_socket_opts(8470, <<"127.0.0.1">>)).
...
%% The same socket options hecate_om_sup gives the real listener, on an
%% ephemeral port.
with_listener(Ip, Check) ->
    Ref = {?MODULE, make_ref()},
    Dispatch = cowboy_router:compile([{'_', []}]),
    {ok, _} = ranch:start_listener(Ref, ranch_tcp, hecate_om_sup:health_socket_opts(0, Ip),
