...
%%% mesh capability `hecate_om:boot/2' advertises for any service
%%% exporting `describe_rpc_capabilities/0' and/or
%%% `describe_pubsub_capabilities/0' (see `hecate_om_service''s own
%%% moduledoc for both callbacks).
%%%
%%% Deliberately stateless: `describe/1' (the handler
%%% `hecate_om_simple_handler' dispatches to) reads the current
%%% service module from `hecate_om:service_module/0' -- the SAME
%%% `persistent_term' `boot/2' already populates for every other
%%% hecate_om internal that needs to know "which service am I" -- so
%%% this needs no gen_server of its own, matching `hecate_om_simple_handler`'s
%%% own "no per-request state" ethos.
-module(hecate_om_describe).

-export([describe/1, capability_for/2]).

%% @doc `hecate_om_simple_handler''s dispatch target: `Payload' is
...
describe_current_service() ->
    ServiceMod = hecate_om:service_module(),
    #{
...
-spec capability_for(module(), binary()) -> hecate_om_service:capability() | undefined.
...
capability_if_any(true, ServiceName) ->
    #{name    => <<ServiceName/binary, ".describe_capabilities">>,
      version => 1,
      handler => {hecate_om_simple_handler, {?MODULE, describe}}}.
